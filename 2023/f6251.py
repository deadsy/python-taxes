...
class F6251(Form):
    ...
    def __init__(f, inputs, f1040, sched_a, sched_d, sched_d_re):
        ...
        if (
            f.rowsum(["2" + chr(x) for x in range(ord("c"), ord("u"))] + ["3"]) or 0
        ) < 0:
            f.must_file = True

        f["4"] = f.rowsum(
            ["2" + chr(x) for x in range(ord("a"), ord("u"))] + ["1", "3"]
        )
        if inputs["status"] == FilingStatus.SEPARATE:
            UPPER = 8 * EXEMPTIONS[inputs["status"]] + EXEMPT_LIMITS[inputs["status"]]
            LOWER = 4 * EXEMPTIONS[inputs["status"]] + EXEMPT_LIMITS[inputs["status"]]
            if f["4"] >= UPPER:
        ...
        # TODO: form 2555
        if (
            (f1040["7"] and not sched_d.mustFile())
            or f1040["3a"]
            or (sched_d_re["15"] > 0 and sched_d_re["16"] > 0)
        ):
            f["12"] = f["6"]
